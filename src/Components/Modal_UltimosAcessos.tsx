        ...
        <div className="p-4 border-b border-gray-100 bg-gray-50 flex flex-col md:flex-row gap-4 justify-between items-center">
            <div className="flex bg-white p-1 rounded-lg border border-gray-200 shadow-sm">
                <button 
                    ...
                    className={`px-4 py-2 rounded-md text-sm font-medium transition ${filtro === 'TODOS' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-50'}`}
                >
                    Todos ({visitantes.length})
                </button>
                <button 
                    onClick={() => setFiltro('DENTRO')}
                    className={`px-4 py-2 rounded-md text-sm font-medium transition ${filtro === 'DENTRO' ? 'bg-green-100 text-green-700' : 'text-gray-600 hover:bg-gray-50'}`}
                >
                    Na Unidade ({visitantes.filter(v => v.status === 'DENTRO').length})
                </button>
                <button 
                    onClick={() => setFiltro('SAIU')}
                    className={`px-4 py-2 rounded-md text-sm font-medium transition ${filtro === 'SAIU' ? 'bg-gray-200 text-gray-700' : 'text-gray-600 hover:bg-gray-50'}`}
                >
                    Finalizados
                </button>
            </div>
